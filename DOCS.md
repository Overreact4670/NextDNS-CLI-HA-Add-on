...
This add-on achieves a rating of **4 out of 6** in Home Assistant's security scoring system.
...
The two point deductions cannot be avoided without fundamentally breaking the add-on's purpose. The AppArmor profile recovers one point and also provides real security value — it prevents the nextdns process from accessing arbitrary parts of the filesystem or making unexpected system calls, even if the container were somehow compromised.
